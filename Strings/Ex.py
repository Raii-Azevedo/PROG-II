...
def repetidas(lst):
    ...
    for i, palavra in enumerate(lst): # Percorre cada palavra na lista de palavras novamente
        for j in range(len(lst)): # Verifica se a palavra atual é igual à palavra em outra posição na lista
            if palavra == lst[j]: # Incrementa o contador correspondente na lista de contadores
                lista_contadores[i] += 1

    for i in range(len(lst)):
        print(f"A palavra '{lst[i]}' apareceu {lista_contadores[i]} vezes na lista.")

def main():
    s = '''Não tinha medo o tal João de Santo Cristo
